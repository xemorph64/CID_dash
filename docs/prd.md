...
| P-05 | What "a network" is (Q-ARC-04) | Case-scoped: two steps from the case's entities; **Louvain** communities for grouping leads (corrected 2026-09-12 — this row said "Leiden", but `architecture.md` §6.7 builds Louvain; master §13.2 allows either, and Louvain ships in NetworkX with a `seed` argument for determinism, with no new dependency. See `docs/NOTES.md` → R-1) | Matches purpose limitation |
