...
- M2: **no source record may contain a true entity id.** Found via the ingest work: `vehicles.jsonl` carried `owner_person_id` (and no owner name at all) and `companies.jsonl` carried `directors[].person_id` beside the surface. That hands M4 the answer it exists to work out, and would let M5's graph writer bypass ER entirely for owner and director edges. Vehicles were worse — with no name there was no surface to resolve, so `truth.json` had zero vehicle mentions and `REGISTERED_TO | Vehicle → Person` (architecture §5.2) could never be built. Identity links belong in `truth.json`; records carry names as written, the way a real RC or company filing does. This is the **second** instance of the ground-truth-in-visible-data bug class (after "N1 Shell 2 Pvt Ltd"), so it now has a scanning invariant test rather than a targeted one.
- M2: IPC 503 *defines* criminal intimidation and 506 *punishes* it; BNS 351 carries both. All four share one `ontology_id`. They had been split, with 503 and 351(2) labelled "anonymous" — that is IPC 507, a different offence not in this synthetic subset. The IPC↔BNS table exists precisely so an old-code case and a new-code case are comparable, so a wrong equivalence defeats its purpose, and MHA/NCRB evaluators know these sections cold.
